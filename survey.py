...
class MultipleChoiceQuestion(Question):
    ...
    id: the id of this question
    text: the text of this question
    options: the answer options of this question
    ...
    def validate_answer(self, answer: Answer) -> bool:
        ...
        question.
        """
        if isinstance(answer.content, str) and answer.content in self.options:
            return True
        return False
...
class NumericQuestion(Question):
    ...
    id: the id of this question
    text: the text of this question
    min_: the minimum possible value of the answer
    max_: the maximum possible value of the answer
    ...
    def __str__(self) -> str:
        ...
        new_question = question + ' between {} and {} inclusive\n'.format(
            self.min_, self.max_)
        return new_question
    ...
    def get_similarity(self, answer1: Answer, answer2: Answer) -> float:
        ...
        step1 = abs(answer2.content - answer1.content)
        step2 = step1 / diff
        step3 = 1.0 - step2
        return step3
...
class YesNoQuestion(MultipleChoiceQuestion):
    ...
    def __str__(self) -> str:
        ...
        You can choose the precise format of this string.
        """
        question = 'Q. ' + self.text + '\n\n' + 'Your answer can only be True'
        new_question = question + ' or False.\n'
        return new_question
...
class CheckboxQuestion(MultipleChoiceQuestion):
    ...
    id: the id of this question
    text: the text of this question
    options: possible answers to this question
    ...
    def validate_answer(self, answer: Answer) -> bool:
        ...
        for i in range(len(answer.content)):
            for j in range(len(answer.content)):
                if i != j and answer.content[i] == answer.content[j]:
                    return False
    ...
    def get_similarity(self, answer1: Answer, answer2: Answer) -> float:
        ...
        unique_strings = []
        common_strings = []
        for s in answer1.content:
            if s not in unique_strings:
                unique_strings.append(s)
            if s in answer2.content and s not in common_strings:
                common_strings.append(s)
        for s2 in answer2.content:
            if s2 not in unique_strings:
                unique_strings.append(s2)
        return len(common_strings) / len(unique_strings)
...
class Survey:
    ...
    def __str__(self) -> str:
        ...
        You can choose the precise format of this string.
        """
        s = 'The questions in this survey are as follows: \n\n'
        for q in self._questions:
            s += 'ID - ' + str(q) + ': ' + str(self._questions[q]) + '\n'
        return s
    ...
    def get_questions(self) -> List[Question]:
        ...
        for id_ in self._questions:
            qs.append(self._questions.get(id_))
        return qs
    ...
    def score_students(self, students: List[Student]) -> float:
        ...
        for qs in questions:
            criterion = self._get_criterion(qs)
            weight = self._get_weight(qs)
            answers = []
            for s in students:
                answers.append(s.get_answer(qs))
            quality_score = criterion.score_answers(qs, answers)
            try:
                new = quality_score * weight
                total_score += new
                total += 1
    ...
    def score_grouping(self, grouping: Grouping) -> float:
        ...
        for group in groups:
            ...
            score += self.score_students(members)
            count += 1
        if len(grouping) == 0:
            return 0.0
        else:
            return score / total_groups
